...
const obtener_direccion_todos_cliente  = async function(req,res){
    if(req.user){
        var id = req.params['id'];
        let dlocal = await Direccion.find({nombres:"Local"});
        let direcciones = await Direccion.find({cliente:id,status:true}).populate('cliente').sort({createdAt:-1});
        direcciones.push(dlocal[0]);
       // Object.assign(direcciones,dlocal);
...
const obtener_detalles_ordenes_cliente  = async function(req,res){
    if(req.user){
        ...
        try {
            let venta = await Venta.findById({_id:id}).populate('admin').populate('admin_envio').populate('direccion').populate('cliente');
